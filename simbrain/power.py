...
class Power(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.total_wire_resistance = wire_resistance_unit * (torch.arange(1, self.shape[1] + 1, device=self.mem_c) + 
        #                                                       torch.arange(self.shape[0], 0, -1, device=self.mem_c)[:, None])
        self.total_wire_resistance = torch.zeros(self.shape[0], device=self.mem_c.device)
    ...
    def set_batch_size(self, batch_size) -> None:
        ...
        self.mem_v_read = torch.zeros(batch_size, *(self.shape[0],1), device=self.mem_v_read.device)
        self.read_Isum = torch.zeros(batch_size, *(self.shape[1],), device=self.read_Isum.device)
        self.selected_write_energy = torch.zeros(batch_size, *(self.shape[1],), device=self.selected_write_energy.device)
    ...
    def read_energy_calculation(self, mem_v_read, mem_c) -> None:
        ...
        self.mem_c = mem_c
        self.dynamic_read_energy += torch.sum(self.mem_v_read * self.mem_v_read * self.wire_cap_cow * self.shape[0])
        mem_v_read_expand = self.mem_v_read.expand(self.mem_c.size(0),self.mem_c.size(1),self.mem_c.size(2))
        self.read_Isum = torch.sum(mem_v_read_expand/(1/self.mem_c + self.total_wire_resistance), dim=1)
        self.static_read_energy += torch.sum(self.read_Isum * self.mem_v_read * self.dt * 1/2)
    ...
    def write_energy_calculation(self, mem_v, mem_c, mem_c_pre) -> None:
        ...
        if self.device_structure == 'trace':
            ...
            self.selected_write_energy = (self.mem_v * self.mem_v * 1/2 * self.dt /(1/2 * (self.mem_c + self.mem_c_pre) + self.total_wire_resistance)).squeeze()
            ...
            for write_row in range(self.shape[0]):
                self.dynamic_write_energy += torch.sum(self.mem_v[:,write_row,:] * self.mem_v.squeeze[:,write_row,:] * self.wire_cap_col)
                self.dynamic_write_energy += torch.max(self.mem_v[:,write_row,:]) * torch.max(self.mem_v[:,write_row,:]) * self.wire_cap_row * (1 + 1/4 * (self.shape[0] - 1))
                self.selected_write_energy = self.mem_v[:,write_row,:] * self.mem_v[:,write_row,:] * 1/2 * self.dt /(1/2 * (self.mem_c[:,write_row,:] + self.mem_c_pre[:,write_row,:]) + self.total_wire_resistance[:,write_row,:])
                ...
                self.half_selected_write_energy = torch.cat([half_selected_write_energy_pre,torch.zeros_like(self.selected_write_energy), half_selected_write_energy_post])
    ...
    def total_energy(self) -> None:
        ...
        self.average_power = self.total_energy / torch.max(self.mem_t)
