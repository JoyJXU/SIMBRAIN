...
class ADC_Module_Power(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dt = memristor_info_dict[self.device_name]['delta_t']
        self.vdd = self.CMOS_tech_info_dict[self.device_roadmap][str(self.CMOS_technode)]['vdd']
        self.relax_ratio_col = self.memristor_info_dict[self.device_name]['relax_ratio_col']
        self.read_v_amp = self.memristor_info_dict[self.device_name]['v_read']
    ...
    def comparator_energy_calculation(self, mem_i_sequence) -> None:
        widthInvN = self.MIN_NMOS_SIZE * self.CMOS_technode_meter
        widthInvP = self.pn_size_ratio * self.MIN_NMOS_SIZE * self.CMOS_technode_meter
        widthNand2N = 2 * self.MIN_NMOS_SIZE * self.CMOS_technode_meter
        widthNand2P = self.pn_size_ratio * self.MIN_NMOS_SIZE * self.CMOS_technode_meter
        widthNand3N = 3 * self.MIN_NMOS_SIZE * self.CMOS_technode_meter
        widthNand3P = self.pn_size_ratio * self.MIN_NMOS_SIZE * self.CMOS_technode_meter

        hInv = self.MAX_TRANSISTOR_HEIGHT * self.CMOS_technode_meter
        hNand2 = self.MAX_TRANSISTOR_HEIGHT * self.CMOS_technode_meter
        hNand3 = self.MAX_TRANSISTOR_HEIGHT * self.CMOS_technode_meter
        
        capInvInput, capInvOutput = self.formula_function.calculate_gate_capacitance('INV', 1, widthInvN, widthInvP, hInv)
        capNand2Input, capNand2Output = self.formula_function.calculate_gate_capacitance('NAND', 2, widthNand2N, widthNand2P, hNand2)
        capNand3Input, capNand3Output = self.formula_function.calculate_gate_capacitance('NAND', 3, widthNand3N, widthNand3P, hNand3)
        
        # INV
        self.comparator_energy += ((capInvInput + capInvOutput) * 4) * self.vdd * self.vdd
		# NAND2
        self.comparator_energy += ((capNand2Input + capNand2Output) * 4) * self.vdd * self.vdd
        # NAND3
        self.comparator_energy += ((capNand3Input + capNand3Output) * 3) * self.vdd * self.vdd
  
        self.comparator_energy *= self.ADC_precision *  self.shape[1] * self.input_bit *  mem_i_sequence.shape[1] * mem_i_sequence.shape[2]

    def mux_energy_calculation(self, mem_i_sequence) -> None:
        ...
        capTgGateN = self.formula_function.calculate_gate_cap(widthTgN)
        capTgGateP = self.formula_function.calculate_gate_cap(widthTgP)
        self.mux_energy += capTgGateN * self.shape[1] * self.vdd * self.vdd;	# Selected pass gates (OFF to ON)
        self.mux_energy += capTgGateP * self.shape[1] * self.vdd * self.vdd;	# Deselected pass gates (ON to OFF)
		
        self.mux_energy *= self.input_bit *  mem_i_sequence.shape[1] * mem_i_sequence.shape[2]
    ...
    def ADC_energy_calculation(self, mem_t) -> None:
        ...
        :param mem_t: Time of the memristor crossbar.
        """
        self.ADC_total_energy = self.shift_add_energy + self.SarADC_energy + self.comparator_energy + self.mux_energy
        ...
        self.sim_power = {'shift_add_energy': self.shift_add_energy,
                          ...
                          'comparator_energy': self.comparator_energy,
                          'mux_energy': self.mux_energy,
