...
class DAC_Module_Power(torch.nn.Module):
    ...
    def switch_matrix_read_initialize(self) -> None:
        ...
        _, self.switch_matrix_read_cap_tg_drain = self.formula_function.calculate_gate_capacitance(gateType='INV', \
                                                        numInput=1, widthNMOS=switch_matrix_read_width_tg_N, widthPMOS=switch_matrix_read_width_tg_P, heightTransistorRegion=switch_matrix_read_height)
    ...
    def switch_matrix_col_write_initialize(self) -> None:
        ...
        _, self.switch_matrix_col_write_cap_tg_drain = self.formula_function.calculate_gate_capacitance(gateType='INV', \
                                                        numInput=1, widthNMOS=switch_matrix_col_write_width_tg_N, widthPMOS=switch_matrix_col_write_width_tg_P, heightTransistorRegion=switch_matrix_col_write_height)
    ...
    def DAC_energy_calculation(self, mem_t) -> None:
        ...
        """
        self.DAC_total_energy = self.switch_matrix_col_write_energy + self.switch_matrix_row_write_energy + self.switch_matrix_read_energy + self.switch_matrix_reset_energy
...
class ADC_Module_Power(torch.nn.Module):
    ...
    def adder_energy_calculation(self, num_adder) -> None:
        ...
        # Second and later stages
        self.adder_energy += (self.adder_cap_nand_output + self.adder_cap_nand_input * 3) * self.vdd * self.vdd * (self.ADC_precision-1)	# #1
        self.adder_energy += (self.adder_cap_nand_output + self.adder_cap_nand_input) * self.vdd * self.vdd * (self.ADC_precision-1)		# #3
        self.adder_energy += (self.adder_cap_nand_output + self.adder_cap_nand_input) * self.vdd * self.vdd * 2 * (self.ADC_precision-1)		# #6 and #7
    ...
    def shift_add_energy_calculation(self, mem_i_sequence) -> None:
        ...
        self.shift_add_energy += self.DFF_energy_calculation(mem_i_sequence.shape[3]*(self.ADC_precision+self.input_bit), self.input_bit) * read_times
