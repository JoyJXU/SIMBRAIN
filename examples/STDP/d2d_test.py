...
parser.add_argument("--d2d_variation", type=int, default=3) # 0: No d2d variation, 1: both, 2: Gon/Goff only, 3: nonlinearity only
...
# %% Multiple test
out_root = 'Accuracy_Results_dynamic_train_d2d_lin_512.txt'
...
for test_cnt in range(multiple_test_no):
    ...
    init_num = 30000 # start capacity (number of training patterns)
    signal_break = 0
    p_break = 0
    tmp_acc = 0
    best_acc = 0
    ...
    for epoch in range(n_epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            if ((epoch * args.n_train + (step + 1) * train_batch_size) >= init_num) and \
                ...
                if (signal_break >= patience) and (p_break >= p):
