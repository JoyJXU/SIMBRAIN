...
"""
改写spikingjelly.clock_driven.surrogate文件，以适应MultiStepLIFNode类的autograd过程
为所有SurrogateClass添加了derivative方法，并实现了torch.jit.script版本
对应于spikingjelly 0.0.0.0.9版
"""
...
class PiecewiseQuadratic(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_PiecewiseQuadratic_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_PiecewiseQuadratic_derivative(x, alpha):
    x_abs = x.abs()
    mask = (x_abs > (1. / alpha))
...
class PiecewiseExp(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_PiecewiseExp_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_PiecewiseExp_derivative(x, alpha):
    return alpha / 2. * (- alpha * x.abs()).exp_()
...
class Sigmoid(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_Sigmoid_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_Sigmoid_derivative(x, alpha):
    sgax = (x * alpha).sigmoid_()
    return (1. - sgax) * sgax * alpha
...
class SoftSign(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_SoftSign_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_SoftSign_derivative(x, alpha):
    return 1. / (2. * alpha * (1. / alpha + x.abs()).pow_(2))
...
class ATan(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_ATan_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_ATan_derivative(x, alpha):
...
class NonzeroSignLogAbs(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_NonzeroSignLogAbs_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_NonzeroSignLogAbs_derivative(x, alpha):
    return 1. / (1. / alpha + x.abs())
...
class Erf(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_Erf_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_Erf_derivative(x, alpha):
    return (- (x * alpha).pow_(2)).exp_() * (alpha / math.sqrt(math.pi))
...
class PiecewiseLeakyReLU(MultiArgsSurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_PiecewiseLeakyReLU_derivative(x, torch.as_tensor(self.w), torch.as_tensor(self.c))


@torch.jit.script
def jit_PiecewiseLeakyReLU_derivative(x, w, c):
    mask_width = (x.abs() < w)
    mask_c = mask_width.logical_not()
    return x.masked_fill(mask_width, 1. / w).masked_fill(mask_c, c)
...
class SquarewaveFourierSeries(MultiArgsSurrogateFunctionBase):
    ...
    def derivative(self, x):
        grad_x = 0.
        w = math.pi * 2. / self.T_period
        for i in range(1, self.n):
            grad_x += torch.cos_((2. * i - 1.) * w * x)
        grad_x *= 4. / self.T_period
        return grad_x
...
class S2NN(MultiArgsSurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_S2NN_derivative(x, torch.as_tensor(self.alpha), torch.as_tensor(self.beta))


@torch.jit.script
def jit_S2NN_derivative(x, alpha, beta):
    sgax = torch.sigmoid(alpha * x)
    grad_x = torch.where(x < 0., alpha * sgax * (1. - sgax), beta / (x + 1.))
    return grad_x
...
class QPseudoSpike(SurrogateFunctionBase):
    ...
    def derivative(self, x):
        return jit_QPseudoSpike_derivative(x, torch.as_tensor(self.alpha))


@torch.jit.script
def jit_QPseudoSpike_derivative(x, alpha):
    return (1. + 2. / (alpha - 1.) * x.abs()).pow_(-alpha)
